    ...
    async def send_all(self, progress_callback=None) -> Dict:
        ...
        while self.usernames_queue:
            ...
            if result == "SUCCESS":
                ...
                # ⏱️ مکث برای جلوگیری از spam detection و report
                # مکث بیشتر = امنیت بیشتر = کمتر ریپورت شدن
                delay = random.uniform(3, 6)  # 3 تا 6 ثانیه بین هر پیام
                logger.info(f"⏱️ Waiting {delay:.1f}s before next message (anti-spam protection)")
                ...
                # مکث کوتاه‌تر برای NOT_FOUND (چون پیام ارسال نشده)
